...
# split the array to a list of arrays along the "id" and "stimclass" dimensions
tempdat <- splitArray(tempdat, c("id", "stimclass"), 
                      list(id = id_dat$group, stimclass = NULL))

# compute grand averages (average across subjects)
tempdat <- lapply(tempdat, avgDims, "id")

# plot the topographies and the centroids at 300 ms, and add the GFP curves 
# of the whole segments
complexplot2dview(
    lapply(tempdat, subsetArray, list(time = "300")), 
    chan_pos, "300", 
    title_row = c("Control", "DL"),
    title_col = c("A", "B", "C"),
    gfp = lapply(tempdat, compGfp))
```
